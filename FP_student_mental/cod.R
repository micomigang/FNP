...
summary(aov(mental_status ~ marital_status*
              gender*age*course*major*study_year*GPA*marital_status,data=data))

cat(colnames(data),sep="*")


#有心理问题的占比
s1 <- sum(data$mental_status==1)/nrow(data)## 64% with mental problem
s2 <- sum(data$treatment==1)/nrow(data)##6% treated in population
s3 <- sum(data$treatment==1)/sum(data$mental_status==1)#9% treated in people who with mental problem 
s4 <- sum(data[data$mental_status==0 & data$treatment == 1,]) ## no one health but treated
pie(c(s1-s2,s2,s3),c("Mentally unhealthy person","Receiving treatment"))
...
ggplot(data_unhealth_plot,aes(x=as.factor(unhealth_grade),y=Number,fill=Symptom))+
  ...
  theme(panel.background = element_rect(fill = "#f5f0ec"),
        ...
        legend.background = element_rect(color = "#f5f0ec", fill = "#f5f0ec"))+
  xlab(label = "Unhelath grade")
...
ggplot(major,aes(x=Major,y=Percent,fill=as.factor(Mental_status)))+
  ...
  theme_classic()+
  scale_fill_discrete(labels=c("Health","Mentally problem"))+
...
ggplot(gender,aes(x=Gender,y=Percent,fill=as.factor(Mental_status)))+
  ...
  theme_classic()+
  scale_fill_discrete(labels=c("Health","Mental problem"))+
  labs(fill="Mental status")+
  theme(panel.background = element_rect(fill = "#f5f0ec"),
        plot.background = element_rect(color = "#f5f0ec", fill = "#f5f0ec"),
        panel.grid.minor = element_blank(),
...
for (i in 1:6){
  ...
  ggsave(filename = file,width = 10, height = 6, dpi = 300,path = savepath,bg="transparent")
  p <- ggplot(plot_data[[i]], aes(var1, var2, fill = corr)) +
...
for (i in 1:6){
  ...
  png(file = file,width = 8,height = 8)
  png(bg = )
